...
var VERSION = execSync(BIN + ' --version').toString().trim();
...
Phapper.prototype = {
    ...
    runSync: function () {
        var result = execSync(this.commandString()+" 2>&1");
        var parsed;

        try { parsed = JSON.parse(result.toString()); } catch(e) {}

        this.results = { parsed: parsed, output: result.toString(),
